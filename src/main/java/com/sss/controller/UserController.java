...
//        //从session中获得用户的基本信息
        ...
        if (StringUtils.getInstance().isNullOrEmpty(name) || StringUtils.getInstance().isNullOrEmpty(level) || StringUtils.getInstance().isNullOrEmpty(remark) || StringUtils.getInstance().isNullOrEmpty(price)
            ...
            return "redirect:/publish_product?error=请输入正确的格式";
        }
        //插入
        if (action == 1) {
            ...
            builder.append(StringUtils.getInstance().getRandomChar()).append(new Date().getTime()).append(".jpg");
            thumbnails.append(builder);
//        String fileName = "\\" + random + ".jpg";
            ...
            if (StringUtils.getInstance().thumbnails(path+random,thumbnails.toString())){
                save = "\\image\\thumbnails\\" + builder.toString();
            } else {
...
//                shopPictureService.deleteByPrimaryKey(spid);
            ...
            //        e.printStackTrace();
            //    }
            //    if(StringUtils.getInstance().thumbnails(oldImage,oldThumbnails)){
            //    }else{
            //        return "redirect:publish_product?error=生成缩略图失败";
            //    }
            //}
